...
ggplot(data = toolik_ts, aes(x = date, y = daily_air_temp)) +
  geom_line() +
  labs(x = "Date",
       y = "Mean daily air temperature (Celsius)\n at Toolik Station") # /n adds a line break to caption
```
...
-   Clear seasonality, with a dominant seasonal feature and also a secondary peak each year - that secondary peak (during summer) has increased substantially
...
```{r}
# Find STL decomposition (Seasonality Trend L (L= Locally Estimated Scatterplot Smoothing))
dcmp <- energy_ts %>%
  filter(sector == 'residential') %>%
